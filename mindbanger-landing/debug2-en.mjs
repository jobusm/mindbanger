...
async function test() {
    ...
    const { data: sourceRow } = await sb.from('daily_signals').select('*').eq('id', 'cff25e84-6fc0-4469-bd0b-201e31caa11b').single();
    let rawText = sourceRow.script;
    console.log('Text length:', rawText.length);

    console.log('2. ElevenLabs API...');
    const start = Date.now();
    const elevenRes = await fetch('https://api.elevenlabs.io/v1/text-to-speech/eGbMQjHtPyAT1vmmz4ux', {
      method: "POST",
      headers: {
    ...
    const uniqueFilename = 'test-en-audio-' + Date.now() + '.mp3';
