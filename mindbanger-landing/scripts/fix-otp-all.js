...
fs.writeFileSync(path.join(__dirname, '../emails/otp-template.html'), otp_html, 'utf8');
console.log('OTP Template HTML fixed!');

// Now fix route.ts translations
const routePath = path.join(__dirname, '../src/app/api/auth/magic-link/route.ts');
let routeCode = fs.readFileSync(routePath, 'utf8');

routeCode = routeCode.replace(/sk: {.*?},\s*cz: {.*?}/s, \`sk: {
...
console.log('Route.ts translations fixed!');
