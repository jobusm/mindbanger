...
const path = 'c:/Users/miros/Documents/Mindbanger.com/src/app/api/webhooks/stripe/route.ts';
...
const replacement1 = `const subscriptionId = session.subscription as string;
        ...
        const refCode = session.metadata?.refCode || session.subscription_data?.metadata?.refCode;
        const refMode = session.metadata?.refMode || session.subscription_data?.metadata?.refMode;
...
const replacement2 = `
          ...
          if (refCode && refMode && refCode !== userId) {
            try {
              // Find affiliate mapped to refCode (which is the user id of the referring affiliate)
